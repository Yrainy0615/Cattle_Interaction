...
def make_parser():
    ...
    parser.add_argument("-c", "--ckpt", default='weights/yolo.pth', type=str, help="ckpt for eval")
    parser.add_argument("-pose", "--pose_checkpoint", default='weights/hrnet.pth', type=str, help="pose ckpt path")
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            ...
            label = ["conflicting","licking","mounting"]
            int_model = resnet50(pretrained=False)
            fc_inputs = int_model.fc.in_features
            int_model.fc = nn.Sequential(
            ...
            int_model.load_state_dict(torch.load('weights/interaction_rgb.pth'))
            int_model.to("cpu")
            int_model.eval()
            two_box = list(itertools.combinations(bbox_list, 2))
            for i in range(len(two_box)):
                Iou = IoU(two_box[i][0],two_box[i][1])
                if Iou >0.01   : #0.01
                    y0 = min(int(two_box[i][0][1]),int(two_box[i][1][1]),int(two_box[i][0][3]),int(two_box[i][1][3]))
                    y1 = max(int(two_box[i][0][1]),int(two_box[i][1][1]),int(two_box[i][0][3]),int(two_box[i][1][3]))
                    x0 = min(int(two_box[i][0][0]),int(two_box[i][1][0]),int(two_box[i][0][2]),int(two_box[i][1][2]))
                    x1 = max(int(two_box[i][0][0]),int(two_box[i][1][0]),int(two_box[i][0][2]),int(two_box[i][1][2]))
                    # print("y0:{},y1:{},x0:{},x1{}".format(y0,y1,x0,x1))
                    # print(two_box[i][0],two_box[i][1])
                    int_area = frame[y0:y1,x0:x1]  #y0:y1,x0:x1
                    int_img = Image.fromarray(int_area)
                    ...
                    int_img = data_transforms(int_img).unsqueeze(0)
                    int_img.to("cpu")
                    #print(int_area.shape)
                    output = int_model(int_img)
                    int_l = torch.argmax(output, dim=1)
                    int_label = label[int(int_l)]
                    cv2.rectangle(frame,(min(int(two_box[i][0][0]),int(two_box[i][1][0])),min(int(two_box[i][0][1]),int(two_box[i][1][1]))), (max(int(two_box[i][0][2]), int(two_box[i][1][2])), max(int(two_box[i][0][3]),int(two_box[i][1][3]))), (0, 0, 255),6)
                    cv2.putText(frame, int_label, (min(int(two_box[i][0][0]),int(two_box[i][1][0])),30+ max(int(two_box[i][0][3]),int(two_box[i][1][3]))), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0,0,255), 6)
